...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        "*** YOUR CODE HERE ***"
        self.startingGameState = startingGameState  # used to test performance of maze distance in corner heuristic
    ...
    def expand(self, state):
        ...
        for action in self.getActions(state):
            ...
            child = (next_state, action, self.getActionCost(state, action, next_state))
    ...
    def getNextState(self, state, action):
        ...
        next_visited_corners = [x for x in state[1]]
        # if we are in a corner and we have not been in the corner before
        if (next_position in self.corners) and (next_position not in state[1]):
            next_visited_corners += [next_position]
...
def cornersHeuristic(state, problem):
    ...
    for corner in problem.corners:
        if corner not in state[1]:
            heuristic = max(heuristic, util.manhattanDistance(state[0],corner))
    return heuristic
...
def foodHeuristic(state, problem):
    ...
    costs = [0]
    costs += [mazeDistance(food_position, position, problem.startingGameState) for food_position in food_list]
    return max(costs)
